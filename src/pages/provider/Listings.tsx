...
import { useMyListings, useCreateProviderListing } from '../../hooks/useListing';
import { Plus, Loader2, Image as ImageIcon, MapPin, List, ChevronDown } from 'lucide-react';
        ...
        <ProviderLayout>
            <div className="flex justify-between items-center mb-8">
                ...
                <button 
                    ...
                    className="bg-button-dark text-white px-5 py-2.5 rounded-full font-medium flex items-center gap-2 hover:bg-button-dark-hover transition-colors shadow-sm"
                >
                            ...
                            <form id="create-listing-form" onSubmit={handleCreateListing} className="space-y-5">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div className="flex flex-col gap-2 relative">
                                        ...
                                        <div 
                                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-[15px] outline-none hover:border-slate-300 transition-all flex items-center justify-between cursor-pointer"
                                        ...
                                        {isTypeDropdownOpen && (
                                            <>
                                                ...
                                                />
                                                <div className="absolute bottom-full left-0 right-0 mb-1 bg-white border border-slate-200 rounded-xl shadow-lg z-20 overflow-y-auto max-h-60 py-1">
                                                    {LISTING_TYPES.map(cat => (
                                                        <div 
                                                            key={cat} 
                                                            className={`px-4 py-3 text-[15px] cursor-pointer hover:bg-slate-50 transition-colors ${type === cat ? 'bg-slate-50 font-medium text-blue' : 'text-slate-700'}`}
                                                            onClick={() => {
                                                                setType(cat);
