...
const AdminLogin = () => {
  ...
  const handleLogin = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (res?.status === "invalid" || !res) {
        ...
        // Success login, redirect to admin dashboard
        toast.success("Admin Login Successful")
        toast.success("Admin Dashboard in Progress")
  ...
  return (
    <AuthLayout illustration={LoginIllustration}>
      <AuthForm>
        <div>
           <h2 className='font-semibold text-3xl mb-1'>Admin Portal</h2>
           <p className='text-slate-500 mb-6'>Sign in to manage the platform.</p>
        </div>
        <div className="flex flex-col gap-6 w-full">
          <Input 
            label="Admin Email" 
            type="email" 
            placeholder="admin@slottr.com" 
            value={email}
